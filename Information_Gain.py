...
def information_gain(matrix, n_sample, input_list, classes, all_ngrams):
    ...
    for i in all_ngrams: # index1=all_ngrams.index(i)
        ...
        for j in classes:  # index2 = classes.index(i)
            ...
            p_c = 1.0 * n_c/ n_sample
            count = matrix[index_r][index_c]
            if count == 0:
                temp = 1.0*math.log(1.0/(p_v0*p_c))
                ig+=temp
            elif count == n_c:
                temp = 1.0*math.log(1.0/(p_v1*p_c))
                ig+=temp
            elif count !=0 and count < n_c:
                p_v1_c = 1.0 * count / p_c
                p_v0_c = 1 - p_v1_c     
                temp = p_v1_c * math.log(p_v1_c/(p_v1*p_c) )+ p_v0_c * math.log(p_v0_c/(p_v0*p_c))
                ig += temp        
        IG.append(ig)
    return IG
            
def classwise_relevant(matrix, all_ngrams):
    top_ngrams = []
    for i in range(matrix.shape[0]): # 40
        cnt_dict = {}
        for j in range(matrix.shape[1]):  # 74152
            cnt_dict[all_ngrams[j]] = matrix[i,j]
        rank_dict_list = sorted(cnt_dict.items(), lambda x, y: cmp(x[1], y[1]), reverse=True)
        for k in rank_dict_list[:8000]:
            top_ngrams.append(k[0])
        print i
    extracted_ngrams = list(set(top_ngrams))
    print len(extracted_ngrams)
...
#        input_list[key] = list(set(val))
   ...
   # print len(classes) 40
    all_ngrams=list(set(ngrams))  # 72152
  #  print len(ngrams) 
  #  print len(set(ngrams))  
    ...
    np.savetxt('/Users/ceciliaLee/Desktop/Windows/matrix_4ngrams.txt', matrix_01, fmt = '%d')

    matrix_01 = np.loadtxt('/Users/ceciliaLee/Desktop/matrix.txt',  dtype=np.int32)
    print matrix_01.shape
    for i in range(matrix_01.shape[0]):
        for j in range(matrix_01.shape[1]):
            if matrix_01[i,j]<0:
                print matrix_01[i,j]
    """
    matrix = np.loadtxt('/Users/ceciliaLee/Desktop/txt/matrix_4ngrams.txt',  dtype=np.int32)
    n_sample = len(sample_list)
    #info_gain = information_gain(matrix_01, n_sample, input_list, classes, all_ngrams)
    extracted_ngrams = classwise_relevant(matrix, all_ngrams)
    f = open("/Users/ceciliaLee/Desktop/txt/extracted_top_4grams8000.txt",'wb')
    for t in extracted_ngrams:
    ...
    f = open("/Users/ceciliaLee/Desktop/rank_ig_4grams.txt",'wb')
    for t in rank_ig:
        line = ' '.join(str(x) for x in t)
        f.write(line + '\n')
    f.close()
